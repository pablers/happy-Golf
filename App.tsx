...
// Este componente contiene la lógica condicional del layout protegido.
const AppContent: React.FC = () => {
  const { userProfile, isLoading, login, enterAsGuest } = useAuth();

  // Mientras validamos token o cargamos perfil, mostramos un spinner global.
  if (isLoading) {
    return (
  ...
  // Si no hay sesión activa, mostramos el formulario de login o acceso invitado.
  if (!userProfile) {
...
const App: React.FC = () => (
  // Componemos los distintos proveedores y el router de la aplicación.
  <BrowserRouter>
    <ThemeProvider>
      <AuthProvider>
        <RoundsProvider>
