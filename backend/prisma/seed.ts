...
async function main() {
  ...
  // --- 4. Seed Rounds ---
  const roundsPath = path.join(__dirname, '../../data/registro-partidas.csv');
  ...
  for (const line of roundLines) {
    ...
    const round = await prisma.round.create({
      data: {
        ...
        practiceTime: practiceTimeMap[rowData.practice_time] || 'none',
        weather: rowData.initial_weather as WeatherCondition,
        wind: rowData.initial_wind as WindCondition,
        userId: user.id,
        ...
        scores: {
          create: INITIAL_HOLE_SCORES.map((hole, index) => ({
            ...
            strokes: rowData[`h${index + 1}_strokes`] ? parseInt(rowData[`h${index + 1}_strokes`], 10) : null,
            putts: rowData[`h${index + 1}_putts`] ? parseInt(rowData[`h${index + 1}_putts`], 10) : null,
            comment: rowData[`h${index + 1}_comment`] || null,
            fairwayHit: rowData[`h${index + 1}_fairwayHit`] === 'true' ? true : rowData[`h${index + 1}_fairwayHit`] === 'false' ? false : null,
          })),
        },
      },
    });

    // Create a corresponding HcpRecord for the user
